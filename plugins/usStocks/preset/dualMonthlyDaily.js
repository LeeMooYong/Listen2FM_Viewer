// Listen2FM_Viewer/plugins/usStocks/preset/dualMonthlyDaily.js
// US 듀얼(월/일) — 1×2(좌: 월봉 / 우: 일봉), 각 컬럼: 메인(캔들+MA+볼륨) + 보조(서브)
// KR 듀얼과 동일 로직/스타일(보조지표 포함) — FG Index 제외
//
// 변경사항(요청 반영):
// - 일봉 보조지표 MA_Oscillator를 '중기(20–60), 5'로 고정
//   · 본선 = (20-60) → 빨강, 1px
//   · 보조선 = (5-60) → 흰색, 1px
//   · 기준선 = 0 → 초록, 1px
//   · 레전드: "MA_Oscillator(20-60), 5" (20=빨강, 60=초록, 5=흰색)
// - 월봉은 기존 3–12 그대로 유지
...
const BARS_MONTH = 45;
const BARS_DAILY = 130;

let __LAST_DISPOSER__ = null;

export async function mountUSDualMonthlyDaily({
  ...
  function showNotice(el, text) {
    el.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#bbb;font-weight:600;">${text}</div>`;
  }

  // 레이아웃
  root.innerHTML = `
    <div id="us-dual-root" style="display:grid;grid-template-columns:1fr 1fr;gap:6px;height:100%;">
      <div id="us-month" style="display:grid;grid-template-rows:3fr 1fr;gap:6px;min-height:120px;">
        <div id="usm-main" style="position:relative;"></div>
        <div id="usm-sub"  style="position:relative;"></div>
      </div>
      <div id="us-daily" style="display:grid;grid-template-rows:3fr 1fr;gap:6px;min-height:120px;">
        <div id="usd-main" style="position:relative;"></div>
        <div id="usd-sub"  style="position:relative;"></div>
      </div>
    </div>
  `;

  // DOM
  const elMonthMain = root.querySelector("#usm-main");
  const elMonthSub = root.querySelector("#usm-sub");
  const elDayMain = root.querySelector("#usd-main");
  const elDaySub = root.querySelector("#usd-sub");

  // 타이틀
  createTitleOverlay(elMonthMain, `${symbol} (Monthly)`);
  createTitleOverlay(elDayMain, `${symbol} (Daily)`);

  // 데이터 로드(가드)
  let md, dd;
  try {
    [md, dd] = await Promise.all([
      loadEquity({ symbol, timeframe: "monthly" }),
      loadEquity({ symbol, timeframe: "daily" }),
    ]);
    if (!Array.isArray(md) || md.length === 0 || !Array.isArray(dd) || dd.length === 0) {
      showNotice(root, "데이터 준비중…");
      __LAST_DISPOSER__ = () => { };
      return __LAST_DISPOSER__;
    }
  } catch (e) {
    console.warn("US dualMonthlyDaily data load error:", e);
    showNotice(root, "데이터 준비중…");
    __LAST_DISPOSER__ = () => { };
    return __LAST_DISPOSER__;
  }
  ...
  function buildMainSet(chart, candles, maDefs) {
    ...
    vol.setData(candles.map(c => ({
      time: c.time, value: c.volume ?? 0, color: c.close >= c.open ? UP : DOWN,
    })));
    ...
    maDefs.forEach(def => {
      const s = chart.addLineSeries({
        ...
        priceLineColor: def.color, lastValueVisible: def.lastValueVisible !== false,
      });
    ...
    const candle = chart.addCandlestickSeries({
      ...
      borderDownColor: DOWN, borderUpColor: UP,
  ...
  try { mm.lines.ma6?.applyOptions({ lineStyle: 2 }); } catch { }

  // 일봉 MAs: 240/120/60/20/5
  ...
  function buildMAOsc(subChart, candles, fast, slow) {
    ...
    const zero = subChart.addLineSeries({ color: "#FFD700", lineWidth: 1, lineStyle: 0, lastValueVisible: false, priceLineVisible: false });
  ...
  // 일봉: 중기(20–60), 보조선(5–60) 추가 + 모든 선 1px
  const do_base = chDS.addBaselineSeries({
    baseValue: { type: "price", price: 0 },
    topFillColor1: "rgba(0, 128, 0, 0.25)",
    topFillColor2: "rgba(0, 128, 0, 0.25)",
    bottomFillColor1: "rgba(239, 83, 80, 0.2)",
  ...
  const do_zero = chDS.addLineSeries({ color: "green", lineWidth: 1, lineStyle: 0, lastValueVisible: false, priceLineVisible: false });
  const do_line = chDS.addLineSeries({ color: "red", lineWidth: 1 });        // (20-60)
  const do_white = chDS.addLineSeries({ color: "white", lineWidth: 1, lastValueVisible: false, priceLineVisible: false }); // (5-60)

  const do_raw = calculateMAOscillator(dd, 20, 60); // (20-60)
  // (5-60) 계산
  const sma5 = calculateSMA(dd, 5);
  const sma60 = calculateSMA(dd, 60);
  const mapTV = arr => new Map(arr.filter(x => Number.isFinite(x?.value)).map(x => [x.time, x.value]));
  const m5 = mapTV(sma5), m60 = mapTV(sma60);
  const do_white_raw = dd
    .map(c => ({ time: c.time, value: (m5.get(c.time) ?? NaN) - (m60.get(c.time) ?? NaN) }))
    .filter(x => Number.isFinite(x.value));

  do_base.setData(padWithWhitespace(dd, do_raw));
  do_line.setData(padWithWhitespace(dd, do_raw));
  do_white.setData(padWithWhitespace(dd, do_white_raw));
  do_zero.setData(dd.map(c => ({ time: c.time, value: 0 })));
  ...
  function buildDisparity(subChart, candles, maPeriod) {
    ...
    const fill = subChart.addBaselineSeries({
      ...
      bottomFillColor1: "rgba(239, 83, 80, 0.2)", bottomFillColor2: "rgba(239, 83, 80, 0.2)",
      topLineColor: "rgba(0,0,0,0)", bottomLineColor: "rgba(0,0,0,0)",
      priceLineVisible: false, lastValueVisible: false,
    });
 ...
 100%{ box-shadow:0 0 0 0 rgba(255,215,0,0); opacity:.85; } }`;
  ...
  function makeDot(hostEl, bg, anim) {
    ...
    Object.assign(d.style, {
      ...
      background: bg, pointerEvents: "none", zIndex: 6,
      animation: `${anim} 1.6s ease-out infinite`, left: "-9999px", top: "-9999px",
    });
    hostEl.appendChild(d);
    return d;
  }
  const dotMO = makeDot(elMonthSub, "#FFD700", "l2fmOscPulse");
  const dotDO = makeDot(elDaySub, "#FFD700", "l2fmOscPulse");
  ...
  const posDO = () => {
    if (daySubCurrent === "MAOSC") return placeDot(chDS, do_line, do_raw, dotDO);
  ...
  const onMonthDbl = () => { setInitialVisibleRange(chMM, md, BARS_MONTH); setTimeout(() => { posMO(); posDISPM(); }, 0); };
  const onDailyDbl = () => { setInitialVisibleRange(chDM, dd, BARS_DAILY); setTimeout(() => { posDO(); posDISPD(); }, 0); };
  ...
  function showDayRSI() {
    ...
    do_base.setData([]); do_line.setData([]); do_white.setData([]); do_zero.setData([]);
  ...
  const teardown = () => {
    ...
    setActive(btnMAO, false); setActive(btnRSI, false);
    setActive(btnDISP, false); setActive(btnMACD, false);
  ...
  __LAST_DISPOSER__ = teardown;
  return teardown;
}

export function dispose() {
  try { __LAST_DISPOSER__?.(); } catch { }
  __LAST_DISPOSER__ = null;
}

export const presetKey = "usDualMonthlyDaily";
