// Listen2FM_Viewer/plugins/usStocks/preset/single30m.js
// US 30분봉 단일 차트 — US Single Daily와 동일 스펙(지표/도트/레전드/깜빡이/동기화)로 이식
// + MA Oscillator 확장: Mid(20–60, 흰=5–60, 교차=5–20) / Long(60–240, 흰=20–240, 교차=20–60) 토글
// - timeframe만 "30m"으로 변경
...
export async function mountUSSingle30m({
    ...
    // 레이아웃: 메인(4) + 서브(1)
    root.innerHTML = `
    <div id="l2fm-us-single30m" style="display:grid;grid-template-rows:4fr 1fr;gap:6px;height:100%;">
    ...
    // 타이틀
    createTitleOverlay(elMain, `${symbol} • 30m`);

    // 데이터 로드
    let candles = [];
    try {
        candles = await loadEquity({ symbol, timeframe: "30m" });
        if (!Array.isArray(candles) || candles.length === 0) {
            showNotice("데이터 준비중…");
            return () => { };
        }
    } catch (e) {
        console.warn("[US Single30m] data load error:", e);
        showNotice("데이터 준비중…");
        return () => { };
    }
    ...
    const positionMAODot = () => { const last = (MAO_MODE === 'long' ? longRed.at(-1) : midRed.at(-1)) || maoscRaw.at(-1); const p = posDot(subChart, maoscLine, last); if (p) { maoDot.style.left = p.x + 'px'; maoDot.style.top = p.y + 'px'; } else maoDot.style.left = maoDot.style.top = '-9999px'; };
    ...
    function renderMAOscillator(mode = 'mid') {
        ...
        if (mode === 'long') {
            // ✅ 장기모드: 60-240(초록 1px), 20-240(빨강 1px), 기준선 0(마젠타 1px)
            ...
            // ✅ 중기모드: 20-60(빨강 1px), 5-60(흰 1px), 기준선 0(초록 1px)
...
export default mountUSSingle30m;
export const presetKey = 'usSingle30m';
