// plugins/usStocks/preset/usQuadMonthlyDailyWeekly60m.js
...
export default async function mount({ mainRoot, symbol = "SPY" } = {}) {
    ...
    const pH = buildPane(root, `${symbol} (60m)`);
    ...
    const [m, d, w, h] = await Promise.all([
        ...
        loadEquity({ symbol, timeframe: "60m" }),  // ← 60분
    ]);
    ...
    // 초기 가시범위
    if (M.c.length) setInitialVisibleRange(pM.chart, M.c, 50);
    if (D.c.length) setInitialVisibleRange(pD.chart, D.c, 130);
    if (W.c.length) setInitialVisibleRange(pW.chart, W.c, 130);
    if (H.c.length) setInitialVisibleRange(pH.chart, H.c, 180);

    // 더블클릭 복귀
    const dbl = (p, data, bars) => p.wrap.addEventListener("dblclick", () => setInitialVisibleRange(p.chart, data, bars));
    dbl(pM, M.c, 50); dbl(pD, D.c, 130); dbl(pW, W.c, 130); dbl(pH, H.c, 180);
